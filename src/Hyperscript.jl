...
Validations other than `ValidateNone` also enforce that numeric attribute values are non-`NaN`.
...
struct ValidateNone <: Validation end

sym2code(::ValidateCombined) = COMBINED_ATTR_NAMES
sym2code(::ValidateHTML) = HTML_ATTR_NAME
sym2code(::ValidateSVG) = SVG_ATTR_NAME

tag2attrs(::ValidateCombined) = COMBINED_ATTRS
tag2attrs(::ValidateHTML) = HTML_ATTRS
tag2attrs(::ValidateSVG) = SVG_ATTRS
...
function validatetag(v::Validation, tag)
    tag ∈ tags(v) || error("$tag is not a valid $(name(v)) tag")
    tag
end
validatetag(v::ValidateNone, tag) = tag
...
validatevalue(v::ValidateNone, tag, attr, value) = value

function validateattrs(v::Validation, tag, nt::NamedTuple)
    sym_to_code = sym2code(v)
    ATTRS = tag2attrs(v)
    attrs = Dict{String, Any}()
    for (sym, value) in pairs(nt)
        attr = get(sym_to_code, sym) do
...
function validateattrs(v::ValidateNone, tag, nt::NamedTuple)
...
function Node(v::V, tag, children, attrs) where {V <: Validation}
    Node{V}(validatetag(v, tag), validateattrs(v, tag, attrs), flat(children))
end
...
function (node::Node{V})(cs...; as...) where {V <: Validation}
    Node{V}(
        tag(node),
        isempty(as) ? attrs(node)    : merge(attrs(node), validateattrs(V(), tag(node), as)),
...
"""
m(tag, children...; attrs...)            = Node(ValidateCombined(), tag, children, attrs)
m_svg(tag, children...; attrs...)        = Node(ValidateSVG(), tag, children, attrs)
m_html(tag, children...; attrs...)       = Node(ValidateHTML(), tag, children, attrs)
m_novalidate(tag, children...; attrs...) = Node(ValidateNone(), tag, children, attrs)
